...
function App() {
  ...
  const data = [
    ...
    let showModal = null
    if (crr){
      showModal = <Modal datum={crr} onClose={()=>{setCrr(null)}} />
      console.log(1)
    }
  return (
    <>
      <div className="header">
        ...
        <span><input type="text" placeholder='Search anime or movie'/></span>
      </div>
      <h4>Explore</h4>
      <p className='what'>What are you gonna watch today ?</p>
      <Carousel key={dataCar.id} image={dataCar.image} name={dataCar.movieName} des={dataCar.description}></Carousel>
      <h4>New Release</h4>
      <div className="cardHolder">
        {data.map((datum) =>{
          return <Card key={datum.id} image={datum.image} epi={datum.episode} name={datum.movieName} onClick={() => {
            setCrr(datum)
            console.log(1)
          }} />
        })}
